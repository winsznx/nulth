// Headless e2e for the Agent Desk: drive the live SSE run (real Claude + server-side prover) from
// the browser, capture the streamed log, payment tx, jailbreak refusal, and backstop tx.
import http from 'http'; import fs from 'fs'; import path from 'path'; import { spawn } from 'child_process';
const ROOT='/Users/mac/covenant/web', OUT='/Users/mac/covenant/web/.shots', PORT=8134;
...
const ud='/tmp/covagent_'+process.pid;
const chrome=spawn(CHROME,['--headless=new','--disable-gpu','--no-sandbox','--disable-dev-shm-usage',`--user-data-dir=${ud}`,'--remote-debugging-port=9388','--window-size=1320,900','about:blank'],{stdio:'ignore'});
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
async function wsu(){ for(let i=0;i<60;i++){ try{ const l=await (await fetch('http://localhost:9388/json/list')).json(); const p=l.find(t=>t.type==='page'&&t.webSocketDebuggerUrl); if(p)return p.webSocketDebuggerUrl;}catch{} await sleep(200);} throw 'no cdp'; }
...
console.log('starting agent run (jailbreak)…');
await ev('App.startAgent(true)');
for(let i=0;i<90;i++){ const d=await ev('App.agent.done'); const er=await ev('App.agent.error'); if(d||er){console.log('done:',d,'error:',er);break;} await sleep(2000); }
console.log(await ev('JSON.stringify({logLines:App.agent.log.length, txs:App.agent.txs, refusal:App.agent.refusal, error:App.agent.error})'));
console.log('log tail:', await ev('App.agent.log.slice(-6).map(l=>l.k+": "+l.t).join(" | ")'));
await snap('agent-live.png');
